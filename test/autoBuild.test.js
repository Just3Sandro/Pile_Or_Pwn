...
describe('autoBuildAtActivation', () => {
  ...
  it('runs make if Makefile exists and compiles src/sim', async () => {
    ...
    const versionCall = this.execSpy.getCalls().some(c => typeof c.args[0] === 'string' && /--version/.test(c.args[0]));
    ...
    const compiledCall = this.execSpy.getCalls().some(c => typeof c.args[0] === 'string' && (/-Wall/.test(c.args[0]) || /asm2json/.test(c.args[0])));
